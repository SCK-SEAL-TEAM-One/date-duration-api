...
func Test_CalculateDuration_Between_Mo_Birthday_And_10_June_2019(t *testing.T) {
	...
	if actualCode := response.StatusCode; expectedCode != actualCode {
		t.Fatalf("Expect code %d but get %d", expectedCode, actualCode)
	}
	if expectedResult != actualResult {
		t.Fatalf("Expect %v but get %v", expectedResult, actualResult)
	}
}
...
func Test_NewYearMonthDay_By_Year_1997_Month_10_Day_16_Should_Get_Time(t *testing.T) {
	...
	if actualResult := yearMonthDay.Time; expectedResult != actualResult {
